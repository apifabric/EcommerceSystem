...
def declare_logic():
    ...
    # Ensures the customer's balance does not exceed their credit limit.
    Rule.constraint(validate=Customer,
                as_condition=lambda row: row.balance <= row.credit_limit,
                error_msg="Customer balance ({row.balance}) exceeds credit limit ({row.credit_limit})")

    # Derives the balance as the sum of Order amount_total where date_shipped is null.
    Rule.sum(derive=Customer.balance, as_sum_of=Order.amount_total, where=lambda row: row.date_shipped is None)

    # Derives the order's amount_total as the sum of item amounts.
    Rule.sum(derive=Order.amount_total, as_sum_of=Item.amount)

    # Calculates the item amount as quantity multiplied by unit price.
    Rule.formula(derive=Item.amount, as_expression=lambda row: row.quantity * row.unit_price)

    # Copies the unit price from Product to Item.
